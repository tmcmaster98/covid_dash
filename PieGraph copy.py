...
callbackData2 = CustomJS(args=dict(source=source2, selectWebsite=selectWebsite2,selectoutput=selectoutput2,Wtd=WtdP,Wntd=WntdP,Ntd=NtdP,Nntd=NntdP,dates_str=dates_str),code="""
                const outputW = selectoutput.value
                const web = selectWebsite.value

                if (web == "worldometer"){
                    if (outputW == "Total Deaths"){
                        source.data = Wtd[dates_str[0]]
                    }
                    else{
                        source.data = Wntd[dates_str[0]]
                        
                    }    
                }
                else{
                    if (outputW == "Total Deaths"){
                        source.data = Ntd[dates_str[0]]
                    }
                    else{
                        source.data = Nntd[dates_str[0]]
                    }
                }        
                      """  )
...
layoutfigure2 = row(pie,column(selectoutput2,selectWebsite2))
